...
`.ores-infra.toml` is the repository discovery contract: `[layout]` declares the Terraform roots, while the provider sections retain the modules-first native Supabase, Cloudflare, and Neon integration locations. Provider-native repository integrations are separate from Terraform root-module selection and remain canonical under `modules/`.

Invariants: no backend blocks in `modules/*`; no committed Terraform state or `.terraform/`; environment-only resources may coexist with module calls in root modules; shared resource graphs belong in child modules; provider-native paths must continue to match the modules-first contract.
